...
export const Route = createFileRoute("/_authenticated/lojas/$id")({
  head: () => ({
    meta: [
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <Link to="/lojas" className="font-mono text-[12px] text-leaf">
            ← Todas as lojas
          </Link>
        }
      />
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard label="Responsável" value={<span className="text-[16px] font-sans">{store?.manager_name ?? "—"}</span>} />
      </div>
          ...
          <div className="divide-y divide-line">
            {stock.map((i) => (
              <div key={i.id} className="flex items-center justify-between px-5 py-3">
                ...
                <Tag tone={i.quantity <= i.critical_quantity ? "crit" : i.quantity <= i.min_quantity ? "warn" : "good"}>
                  {num(i.quantity)} un
                </Tag>
              </div>
            ))}
