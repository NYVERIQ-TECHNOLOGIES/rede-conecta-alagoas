...
function Desempenho() {
  ...
  return (
    <>
      <PageHeader eyebrow="📈 Desempenho" title="Análises da Rede" description="Comparações entre lojas, categorias e cooperativas — para decidir com clareza." />
        ...
        {PERIODS.map((p) => (
          <button
            ...
            className={cn(
              ...
              period === p.key ? "bg-leaf text-primary-foreground" : "border border-line text-muted-foreground",
            )}
          >
            {p.label}
          </button>
      ...
      <Panel title="Curva de vendas" subtitle="Evolução diária no período" padded={false}>
        {daily.length === 0 ? (
          ...
          <div className="flex h-48 items-end gap-1 px-5 py-5">
            {daily.map((d) => (
              <div key={d.day} className="group relative flex-1" title={`${d.day} · ${brl(d.value)}`}>
                <div className="w-full rounded-t bg-leaf/70 group-hover:bg-leaf" style={{ height: `${Math.max(4, d.pct)}%` }} />
              </div>
            ))}
          </div>
        )}
