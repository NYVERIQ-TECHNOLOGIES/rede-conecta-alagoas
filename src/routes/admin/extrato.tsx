...
export const Route = createFileRoute("/admin/extrato")({
  head: () => ({
    meta: [
        ...
        {rows.length === 0 ? (
          ...
          <div className="divide-y divide-line">
            {rows.map((s) => (
              <div
                key={s.id}
                className="flex flex-wrap items-center justify-between gap-3 px-5 py-3 font-mono text-[12px]"
              >
                ...
                <Tag
                  tone={
                    s.status === "repassado" ? "good" : s.status === "aguardando" ? "warn" : "crit"
                  }
                >
                  {s.status}
                </Tag>
