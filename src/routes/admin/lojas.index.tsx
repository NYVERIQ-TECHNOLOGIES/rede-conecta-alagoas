...
import { useStores, useInventory, useSales, useCreateStore, useDeleteStore } from "@/lib/queries";
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
          {(stores ?? []).map((store) => {
            ...
            return (
              <div
                key={store.id}
                className="panel rise relative p-5 transition-colors hover:border-leaf/40"
              >
                <Link to="/admin/lojas/$id" params={{ id: store.id }} className="block">
