  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Entrada de Produtos — ALAGOAS+COOPERATIVA" },
      {
        ...
        content: "Recebimento de produtos das cooperativas nas lojas.",
      },
...
function Entradas() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { error } = await supabase.rpc("register_entry", {
      ...
      _expires_at:
        form.expires_at || new Date(Date.now() + 180 * 86_400_000).toISOString().slice(0, 10),
        ...
        <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) => set("produced_at", e.target.value)}
            />
          </label>
          <label className="space-y-1.5">
          ...
          <label className="space-y-1.5 sm:col-span-2">
            ...
            <input
              ...
              placeholder="Nota, recibo ou termo de consignação"
            />
          </label>
