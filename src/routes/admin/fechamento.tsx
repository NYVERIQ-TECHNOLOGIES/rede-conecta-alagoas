...
export const Route = createFileRoute("/admin/fechamento")({
  head: () => ({
    meta: [
...
function Fechamento() {
  ...
  const list = useMemo(
    () =>
      (sales ?? []).filter(
        (s) => (store ? s.store_id === store : true) && s.status === "concluida",
      ),
    [sales, store],
  );
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={num(
            list.reduce((a, s) => a + s.sale_items.reduce((x, i) => x + i.quantity, 0), 0),
          )}
        />
      </div>
