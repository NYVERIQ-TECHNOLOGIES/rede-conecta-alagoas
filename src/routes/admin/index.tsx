  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">
        <StatCard
          label="Clientes"
          value={num(customers?.length ?? 0)}
          hint="Cadastrados na rede"
          tone="leaf"
        />
        <StatCard
          label="Pedidos"
          value={num(orders?.length ?? 0)}
          hint={`${num((orders ?? []).filter((o) => (o as { status?: string }).status === "cancelado").length)} cancelados`}
        />
        <StatCard
          label="Municípios"
          value={num(
            new Set([
              ...(cooperatives ?? []).map((c) => c.city),
              ...(stores ?? []).map((s) => s.city),
            ]).size,
          )}
          hint="Cooperativas e lojas participantes"
        />
      </div>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Link
          to="/admin/pedidos"
          className="panel flex items-center justify-between p-4 transition-colors hover:border-leaf/40"
        >
          <div>
            <div className="label-mono">Pedidos pendentes</div>
            <div className="mt-2 font-display text-[22px] font-bold text-primary">
              {num(
                (orders ?? []).filter(
                  (o) =>
                    (o as { status?: string }).status !== "concluido" &&
                    (o as { status?: string }).status !== "cancelado",
                ).length,
              )}
            </div>
          </div>
          <span className="text-[11px] text-leaf">Ver →</span>
        </Link>
        <Link
          to="/admin/municipios"
          className="panel flex items-center justify-between p-4 transition-colors hover:border-leaf/40"
        >
          <div>
            <div className="label-mono">Atuação territorial</div>
            <div className="mt-2 font-display text-[22px] font-bold text-primary">
              {num(
                new Set([
                  ...(cooperatives ?? []).map((c) => c.city),
        ...
        >
          <div>
            <div className="label-mono">Usuários</div>
            <div className="mt-2 font-display text-[22px] font-bold text-primary" />
          </div>
          <span className="text-[11px] text-leaf">Administrar →</span>
        </Link>
        <Link
          to="/admin/logs"
          className="panel flex items-center justify-between p-4 transition-colors hover:border-leaf/40"
        >
          <div>
            <div className="label-mono">Atividade recente</div>
            <div className="mt-2 font-display text-[22px] font-bold text-primary" />
          </div>
          <span className="text-[11px] text-leaf">Ver logs →</span>
        </Link>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <div className="space-y-3 lg:col-span-2">
          <Panel
            ...
            action={
              <span className="font-mono text-[11px] text-muted-foreground">Vendas no período</span>
            }
            padded={false}
          >
            {isLoading ? (
          ...
          <Panel
            ...
            action={
              <Link
                to="/admin/produtos"
                className="font-mono text-[12px] text-leaf hover:text-leaf-2"
              >
                Ver todos →
              </Link>
            }
